...
- **buttonDelete** - кнопка удаления из корзины


### Класс WebLarekAPI
Класс представления элементов корзины:

- **получить список товаров с сервера** - getProductList(): Promise<IProductItem[]>
- **отправить результат заказа** - orderResult(order: IOrder): Promise
